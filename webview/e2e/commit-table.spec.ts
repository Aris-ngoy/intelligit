...
test.describe("Git log commit table", () => {
	test("renders separate resizable columns with stacked author info", async ({
		page,
	}) => {
		...
		await expect(page.getByTestId("commit-table-column-graph")).toContainText(
			"Graph",
		);
		await expect(page.getByTestId("commit-table-column-message")).toContainText(
			"Message",
		);
		await expect(page.getByTestId("commit-table-column-author")).toContainText(
			"Author",
		);
		await expect(page.getByTestId("commit-table-column-date")).toContainText(
			"Date",
		);
		...
		const graphBox = await page
			.getByTestId("commit-table-column-graph")
			.boundingBox();
		const messageBox = await page
			.getByTestId("commit-table-column-message")
			.boundingBox();
		const authorBox = await page
			.getByTestId("commit-table-column-author")
			.boundingBox();
		const dateBox = await page
			.getByTestId("commit-table-column-date")
			.boundingBox();
		...
		expect(graphBox?.x + graphBox?.width).toBeLessThanOrEqual(
			messageBox?.x + 2,
		);
		...
		const authorWidthAfter = (
			await page.getByTestId("commit-table-column-author").boundingBox()
		)?.width;
