...
suite('Git Log Parser', () => {
	...
	test('assigns graph lanes for linear history', () => {
		const makeRecord = (hash: string, parent: string) =>
			[hash, hash.slice(0, 7), parent, 'A', 'a@b.c', '1700000000', 'msg', '', ''].join(
				GIT_LOG_FIELD_SEP,
			) + GIT_LOG_RECORD_SEP;

		const raw =
			makeRecord('cccccccccccccccccccccccccccccccccccccccc', 'bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb') +
			makeRecord('bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb', 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa') +
	...
	test('formatCommitDate uses Today for same-day commits', () => {
		...
		const todayTs = Math.floor(new Date(2026, 5, 19, 9, 15, 0).getTime() / 1000);
